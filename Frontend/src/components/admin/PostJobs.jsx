...
const PostJobs = () => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${JOB_API}/postJobs`, input, {
        headers: {
      ...
      toast.error(
        error.response?.data?.message || "Failed to post the job. Please check all fields."
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="space-y-2 pb-6 border-b border-gray-100">
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">
                Post a <span className="bg-gradient-to-r from-[#6A38C2] to-indigo-600 bg-clip-text text-transparent">New Job</span> Opening
              </h1>
              <p className="text-sm text-gray-500 max-w-lg">
                Specify role requirements, compensation, and target experience to find the best candidates.
              </p>
            </div>

            {/* No Companies Warning */}
            {companies.length === 0 && (
              <div className="flex items-start gap-3 p-4 bg-amber-50/80 border border-amber-200/70 rounded-2xl text-amber-800 text-sm">
                <AlertCircle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                <div className="flex-1">
                  <p className="font-semibold">No registered companies found</p>
                  <p className="text-xs text-amber-700 mt-0.5">
                    You need to register at least one company before you can post a job.
                  </p>
                </div>
                <Button
                  size="sm"
                  onClick={() => navigate("/admin/companies/create")}
                  className="bg-amber-600 hover:bg-amber-700 text-white text-xs rounded-xl shadow-xs"
                >
            ...
            <form onSubmit={submitHandler} className="space-y-6">
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="" disabled>
                        Choose your registered company
                      </option>
                      {companies.map((comp) => (
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  onChange={changeEventHandler}
                  placeholder="Outline key responsibilities, requirements, expectations, and perks for applicants..."
